...
<div itemscope itemtype="https://schema.org/Product">
	...
	<div class="primary_block row">
		...
		<div class="manufacturer">
			<h1 itemprop="name" class="col-md-8">{$product->name|escape:'html':'UTF-8'}
			
			</h1>
			<div class="logo-manufacturer col-md-4">
			<img src="/img/m/{$product->id_manufacturer}.jpg" class=""  title="" />
			</div>
		</div>	
		<!-- left infos-->
		<div class="pb-left-column col-xs-12 col-sm-8 col-md-8">
				...
				{if $have_image}
					...
					<li class="homeslider-container">
						{if $jqZoomEnabled && $have_image && !$content_only}
							<a class="jqzoom" title="{if !empty($image.legend)}{$image.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" rel="gal1" href="{$link->getImageLink($product->link_rewrite, $image.id_image, 'thickbox_default')|escape:'html':'UTF-8'}">
								<img itemprop="image" src="{$link->getImageLink($product->link_rewrite, $image.id_image, 'large_default')|escape:'html':'UTF-8'}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}"/>
							</a>
						{else}
			...
			<div class="view-wishlist">
						{if isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS}{$HOOK_PRODUCT_ACTIONS}{/if}
							{if !$jqZoomEnabled}
								<span id="zoom" class="span_link no-print" ></span>
							{/if}
			</div>
						...
						{if isset($images)}
							...
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a data-slide-index="{$i}" {if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" {else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			<p class="features">
					...
					{if $feature.name=="Diamètre" || $feature.name=="Mouvement" || $feature.name=="Etanchéité" || $feature.name=="Collection" ||$feature.name=="Matière" }<b>{$feature.name}</b> : {$feature.value}{if $feature.name=="Etanchéité"}m{/if}  <br> {/if}
					{/foreach}
			</p>
			<p class="savoir-plus"><a href="#caracteristiques">{l s="en savoir plus"}</a></p>
			
			{if $baguier=="oui"}
			<p class="savoir-plus"><a href="/img/baguier.jpg" class="baguier">{l s="telecharger notre baguier"}</a></p>
			{/if}
							...
							<div>
								<p class="our_price_display" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
									...
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										<div class="align-right paiement-10">{l s="ou"} 10x{convertPrice price=$productPrice/10} {l s="sans frais"}</div>
<!--
...
-->
					...
					<div class="product_attributes clearfix">
						<div class="pas-de-frais-livraison">
						{l s="pas de frais de livraison"}<br>
						{l s="Expédition sous 10 jours"}
						</div>
						<!-- quantity wanted -->
						{if !$PS_CATALOG_MODE}
					...
					<div class="box-cart-bottom">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print">
								<button type="submit" name="Submit" class="btn-add-cart">
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		<section class="print-social">
			...
			<span class="partage">{l s="partager"}</span>
		</section>
			<!-- More info -->
			<a name="caracteristiques"></a>
			<div class="page-product-heading">{l s='Data sheet'}</div>
			<div class="page-product-box">
		{if isset($product) && $product->description}
				<!-- full description -->
				<h3  class="rte">{$product->description_short}</h3>
...
{* DONNEES CARACTERISTIQUES *}
		{if isset($features) && $features}
			...
			<!-- Data sheet -->
			<table class="features-list-mobile">
				<tr class="item">
					<td class="etiquette" valign="middle">{l s="Marque"}</td><td class="value" valign="middle">{$product_manufacturer->name}</td>
				</tr>
				<tr class="item">
					<td class="etiquette" valign="middle">{l s="Reference"}</td><td class="value" valign="middle">{$product->reference}</td>
				</tr>
				{foreach from=$caracteristiques item=caracteristique key=i}
				<tr class="item">
					<td class="etiquette" valign="middle">{$caracteristique.name}</td><td class="value" valign="middle">{$caracteristique.value}</td>
				</tr>
				{/foreach}
...
{assign var=limit value=2}
			<div class="features-list">
				{if $type_produit=="montre"}
				<div class="bloc">
					<div class="item "><div class="etiquette">{l s="Marque"}</div><div class="value">{$product_manufacturer->name}</div></div>
					<div class="item"><div class="etiquette">{l s="Reference"}</div><div class="value">{$product->reference}</div></div>
					{for $i=0 to $limit}
					<div class="item {if $i==3}last{/if}"><div class="etiquette">{$caracteristiques.$i.name}</div><div class="value">{$caracteristiques.$i.value}</div></div>
					{/for}
				</div>
				<div class="bloc">
				{foreach from=$caracteristiques item=caracteristique key=i}
				{if $i>$limit && $caracteristique.name|lower!="fonctions"}
					<div class="item {if $i==$caracteristiques|count-1}last{/if}">
						<div class="etiquette">{$caracteristique.name}</div>
						<div class="value">{$caracteristique.value}</div>
					</div>
				{/if}
				{/foreach}
				</div>
				{foreach $caracteristiques as $caracteristique}
				{if $caracteristique.name|lower=="fonctions"}
				<div class="bloc-large">
					<div class="etiquette">{$caracteristique.name} :</div><div class="value">{$caracteristique.value|replace:"\n":"  /  "}</div>
				...
				{else}
				<div class="bloc">
					<div class="item "><div class="etiquette">{$caracteristiques.0.name}</div><div class="value">{$caracteristiques.0.value}</div></div>
					<div class="item"><div class="etiquette">{l s="Reference"}</div><div class="value">{$product->reference}</div></div>
					{for $i=1 to 2}
					<div class="item"><div class="etiquette">{$caracteristiques.$i.name}</div><div class="value">{$caracteristiques.$i.value}</div></div>
					{/for}
				...
				<div class="bloc">
					<div class="item">
						<div class="etiquette">{l s="taille"} :</div>
						<div class="value">
						{foreach from=$groups key=id_attribute_group item=group}
							{if $group.attributes|@count}
								{assign var="groupName" value="group_$id_attribute_group"}
