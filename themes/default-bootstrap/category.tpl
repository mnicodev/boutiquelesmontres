...
{if isset($category)}
	{if $category->id AND $category->active}
    	{if $scenes || $category->description || $category->id_image || $category->name}
			<div class="content_scene_cat">
				...
				{else}
                    <!-- Category image -->
                    <div class=" content_scene_cat_bg"{if $category->id_image} style="background:url({$link->getCatImageLink($category->link_rewrite, $category->id_image, 'medium_default')|escape:'html':'UTF-8'}) right center no-repeat;  "{/if}>
		...
		{if isset($subcategories) && count($subcategories)>10000000}
...
<script>
function voir_filtre(o) {
		voir=$("#"+o).is(":visible");
		$(".layered_filter_ul").hide();
		if(voir)$("#"+o).hide();else $("#"+o).show();
