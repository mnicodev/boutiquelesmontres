...
{/if}
	<p class="buttons_bottom_block" id="add2wl">
		<a href="javascript:;" class="{if $themeChoice eq 0}button{else}btn btn-default button button-medium{/if}" onclick='WishlistCartpro("wishlist_block_list", "add", "{$id_product|intval}", $("#idCombination").val(), $("#quantity_wanted").val(), "", "{$modulename|escape:'htmlall':'UTF-8'}","{l s='You need to be logged to add products to your wishlist' mod='blockwishlistpro'}","{l s='Please create your list before adding products (my account/my lists)' mod='blockwishlistpro'}","","{l s='Product added to your list!' mod='blockwishlistpro'}");'><span style='margin:0'>{l s='Add to my wishlist' mod='blockwishlistpro'} <i class='icon-gift'></i></span>
		</a>
	</p>
{if isset($wishlists) && $wishlists && $wishlists|count > 1}
</div>
