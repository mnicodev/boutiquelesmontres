				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						{if isset($filter.slider)}
							<div class="layered_{$filter.type} layered_filter" style="display: none;">
						{else}
							<div class="layered_filter">
						{/if}
                        <div class="layered_subtitle_heading">
                            <span class="layered_subtitle" onclick="voir_filtre($(this).attr('data-id'));" data-id="ul_layered_{$filter.type}_{$filter.id_key}">{$filter.name|escape:'html':'UTF-8'}</span>
								...
								{else}
									...
									{else}
										<select class="select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
											...
											{if $value.nbr || !$hide_0_values}
												<option style="color: {if isset($value.color)}{$value.color}{/if}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}_{$filter.id_key}"  {if !$value.nbr}disabled="disabled"{/if}>
													{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
												</option>
											{/if}
											{/foreach}
...
{l s='Enabled filters:' mod='blocklayered'}
						<a href="#" onclick="window.location='{$base_dir}{$category->link_rewrite}-{$category->id}#'"  title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>
					</span>
					<ul>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>
													{if $filter.format == 1}
