...
class ProductsCategory extends Module
{
	...
	public function hookProductFooter($params)
	{
		...
		if (!$this->isCached('productscategory.tpl', $this->getCacheId($cache_id)))
		{
			...
			// Get infos
			$category_products = $category->getProducts($this->context->language->id, 1, 3); /* 100 products max. */
			...
			$id_cat=(int)$params["product"]->id_category_default;
			...
			foreach($category_products as $key=>$val) {
				$m=new Manufacturer($val["id_manufacturer"]);
				$category_products[$key]["marque"]=$m->name;
			}
