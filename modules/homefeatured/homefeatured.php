...
class HomeFeatured extends Module
{
	...
	public function hookDisplayHome($params)
	{
		...
		{
			$this->_cacheProducts();
			$home_category=Category::getHomeCategories($params["cookie"]->id_lang);
			$liste=HomeFeatured::$cache_products;
			$hc=array();
			$cat_title=array("Montres","Joaillerie");
			$bouton_tout=array("Voir toutes nos montres","Voir tous nos bijoux");
			foreach($home_category as $c) {
				$hc[$c["name"]]=array();
				foreach($liste as $item) {
					$test=Db::getInstance()->ExecuteS("select * from "._DB_PREFIX_."category_product where id_product='".$item["id_product"]."' and id_category='".$c["id_category"]."'");
					if(count($test)) {
