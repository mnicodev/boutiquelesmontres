...
(function ($, window, document) {
	...
	******************************************************************************/
	...
	Zoomer.prototype = {
		...
		***********************************************************************************************************************/
        ...
        ChangeImg: function(params) {
            ...
            var tm = $('<img/>').attr({
                src: $(options.obj).attr('src')
            }).hide().appendTo('div.smooth_zoom_preloader');
            ...
            //Resize the image and position it centered inside the wrapper
            if (self.init_pos == '') {
                self._x = self.tX = (self.sW - self._w) / 2;
                self._y = self.tY = (self.sH - self._h) / 2;
            } else {
                self._x = self.tX = (self.sW / 2) - parseInt(self.init_pos[0]) * self._sc;
                self._y = self.tY = (self.sH / 2) - parseInt(self.init_pos[1]) * self._sc;
                self.oX = (self.tX - ((self.sW - self._w) / 2)) / (self._w / self.sW);
                self.oY = (self.tY - ((self.sH - self._h) / 2)) / (self._h / self.sH);
            }
	...
	$.fn.smoothZoom = function (params, extra) {
		var self = this;
		var l = self.length;
		...
		for (var i = 0; i<l; i++) {
			...
			// Case 2: If the instance already available - Check for method call
                 ...
                 */
					...
					return instance[params].call(instance, extra);
				/**
                 * /PRESTEAMSHOP SOURCE
                 */
