...
class BlockCategories extends Module
{
	...
	/* hook spécial page d'accueil (netconcept, les entrées) */
	public function hookDisplayTopColumn($params) {
		$category = new Category((int)Configuration::get('PS_HOME_CATEGORY'), $this->context->language->id);
		$lct=$category->getSubCategories($category->id,true);
		foreach($lct as $id=>$c) {
			$cat=new Category($c["id_category"]);
			$lct[$id]["link_rewrite"]=$cat->link_rewrite[$params["cookie"]->id_lang];
			$lct[$id]["description"]=$cat->description[$params["cookie"]->id_lang];
		}
		$this->smarty->assign("categories",$lct);

		return $this->display(__FILE__, 'blockcategories_entrees.tpl', $cacheId);
